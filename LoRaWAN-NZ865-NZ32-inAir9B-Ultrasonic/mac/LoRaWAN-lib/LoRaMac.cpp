 ...
 * Data rates table definition
 */
const uint8_t Datarates[]  = { 12, 11, 10,  9,  8,  7,  7, 50 }; /** NB - The numbers are Spreading Factors applicable to each Data Rate from 0 to 7 (7 is FSK) */

/*!
 * Maximum payload with respect to the datarate index. Cannot operate with repeater.
 */
const uint8_t MaxPayloadOfDatarate[] = { 51, 51, 51, 115, 242, 242, 242, 242 };  /** Payload for Data Rate from 0 to 7 */
...
/*!
 * LoRaMAC channels array
 * NB - Channels below are defined in LoRaMac-definitions.h
 * #define LC1                { 865000000, { ( ( DR_5 << 4 ) | DR_0 ) }, 1 }
 * #define LC2                { 865200000, { ( ( DR_5 << 4 ) | DR_0 ) }, 1 }
 * #define LC3                { 865400000, { ( ( DR_5 << 4 ) | DR_0 ) }, 1 }
 *  Configure EU Channels to support NZ865 (KotahiNet Frequencies)
 * #define LC4                { 866200000, { ( ( DR_5 << 4 ) | DR_0 ) }, 0 }
 * #define LC5                { 866400000, { ( ( DR_5 << 4 ) | DR_0 ) }, 0 }
 * #define LC6                { 866600000, { ( ( DR_5 << 4 ) | DR_0 ) }, 0 }
 * #define LC7                { 866800000, { ( ( DR_5 << 4 ) | DR_0 ) }, 0 }
 * #define LC8                { 867000000, { ( ( DR_5 << 4 ) | DR_0 ) }, 0 }
 */
static ChannelParams_t Channels[LORA_MAX_NB_CHANNELS] =
{
    LC1,
...
#endif  // frequency band selection
...
/**
* Re-sets the default value of the US915 channel mask
* Enable 8 125K channels from 921.0 MHz to 922.4 MHz
* Enable 1 500K channel at 922.3 MHz
*/
static void reenableUS915Channels( void ) {

    LoRaMacParams.ChannelsMask[0] = 0x0000;
    LoRaMacParams.ChannelsMask[1] = 0xE000;
    LoRaMacParams.ChannelsMask[2] = 0x001F;
    LoRaMacParams.ChannelsMask[3] = 0x0000;
    LoRaMacParams.ChannelsMask[4] = 0x0010; //922.3MHz
...
static LoRaMacStatus_t ScheduleTx( )
{
    ...
    return SendFrameOnChannel( Channels[Channel] ); // Try to send now
    /*
    * NB - Duty Cycle is off for NZ865
...
LoRaMacStatus_t PrepareFrame( LoRaMacHeader_t *macHdr, LoRaMacFrameCtrl_t *fCtrl, uint8_t fPort, void *fBuffer, uint16_t fBufferSize )
{
    ...
    switch( macHdr->Bits.MType )
    {
        ...
        case FRAME_TYPE_DATA_UNCONFIRMED_UP:
            ...
            // Copy the MAC commands which must be re-sent into the MAC command buffer
...
LoRaMacStatus_t LoRaMacInitialization( LoRaMacPrimitives_t *primitives, LoRaMacCallback_t *callbacks )
{
    ...
    DutyCycleOn = false;
#elif defined( USE_BAND_868 )
   // DutyCycleOn = true; // false for NZ865
    DutyCycleOn = false;
#elif defined( USE_BAND_915 ) || defined( USE_BAND_915_HYBRID )
    DutyCycleOn = false;
#else
    ...
    * Enable 8 125K channels from 921.0 MHz to 922.4 MHz
    * Enable 1 500K channel at 922.3 MHz
    * Note that the values are re-enabled in two other places
    * by reenableUS915Channels() which sets the ChannelsMask  
    */
    LoRaMacParamsDefaults.ChannelsMask[0] = 0x0000;
    LoRaMacParamsDefaults.ChannelsMask[1] = 0xE000;
    LoRaMacParamsDefaults.ChannelsMask[2] = 0x001F;
    LoRaMacParamsDefaults.ChannelsMask[3] = 0x0000;
    LoRaMacParamsDefaults.ChannelsMask[4] = 0x0010;
    LoRaMacParamsDefaults.ChannelsMask[5] = 0x0000;
#elif defined( USE_BAND_915_HYBRID )
    ...
    for( uint8_t i = LORA_MAX_NB_CHANNELS - 8; i < LORA_MAX_NB_CHANNELS; i++ )
    {
        ...
        Channels[i].Frequency = 915.9e6 + (( i - ( LORA_MAX_NB_CHANNELS - 8 ) ) * 1.6e6); // i.e. from 915.9 to 927.1 MHz
...
LoRaMacStatus_t GetMacStatus(void) {
    return  (LoRaMacStatus_t) LoRaMacState;
}

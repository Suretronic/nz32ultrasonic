...
/** Disable and Enable Radio IRQs in support of deepsleep.
*   Avoids radio IRQs waking MCU whilst radio is not fully asleep
*/

void SX1276::DisableRadioIRQs(void) {
    this->dio0.disable_irq();
    this->dio1.disable_irq();
    this->dio2.disable_irq();
    this->dio3.disable_irq();
}

void SX1276::EnableRadioIRQs(void) {
    this->dio0.enable_irq();
    this->dio1.enable_irq();
    this->dio2.enable_irq();
    this->dio3.enable_irq();
}
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
                        ...
                        uint8_t hopPeriod, bool iqInverted, uint32_t timeout )
{
    ...
    // RF_PACONFIG_PASELECT_PABOOST is defined as 0x80
    // RF RF_PACONFIG_MAX_POWER_MASK is defined as 0x8F
    paConfig = ( paConfig & RF_PACONFIG_PASELECT_MASK ) | GetPaSelect( this->settings.Channel );
    paConfig = ( paConfig & RF_PACONFIG_MAX_POWER_MASK ) | 0x70;
    // paConfig = ( paConfig & RF_PACONFIG_MAX_POWER_MASK ) | RF_PACONFIG_PASELECT_PABOOST; // switches PA_BOOST on

    if( ( paConfig & RF_PACONFIG_PASELECT_PABOOST ) == RF_PACONFIG_PASELECT_PABOOST ) // if PA_BOOST is configured on
    {
        if( power > 17 )
        {
            paDac = ( paDac & RF_PADAC_20DBM_MASK ) | RF_PADAC_20DBM_ON;  //RF_PADAC_20DBM_ON defined as 0x07
        }
        else
        {
            paDac = ( paDac & RF_PADAC_20DBM_MASK ) | RF_PADAC_20DBM_OFF; //RF_PADAC_20DBM_OFF defined as 0x04
        }
    ...
    //----------------------- Max Power ON ------------------------------------------------------------
    // paConfig = ( paConfig & RF_PACONFIG_MAX_POWER_MASK ) | 0x8F; // PA_BOOST on - max power
    // paDac = ( paDac & RF_PADAC_20DBM_MASK ) | RF_PADAC_20DBM_ON;  //RF_PADAC_20DBM_ON defined as 0x07
    //--------------------------------------------------------------------------------------------------
